...
documentation using [Plain Help Center (/llms.txt)](https://help.plain.com/llms.txt), and see if
there are any obvious changes that need to be made to the documentation given the changes. If so,
output a new local HTML light theme single page that describes the list of necessary changes, where
each change is a section, which includes:
